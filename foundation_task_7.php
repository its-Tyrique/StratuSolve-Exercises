<?php
    ...
    class Person {
        ...
        /*function savePerson($PersonIdInt, $FirstNameStr, $SurnameStr, $DateOfBirthStr, $EmailAddressStr, $AgeInt) {
            $SavePersonStmt = $this->DBConnectObj->prepare("
                        ...
                        WHERE Id = ?
                    ");
            $SavePersonStmt->bind_param("ssssii", $FirstNameStr, $SurnameStr, $DateOfBirthStr, $EmailAddressStr, $AgeInt, $PersonIdInt);

            if (!$SavePersonStmt->execute()) {
                die(json_encode(["SavePerson execute failed: " . $SavePersonStmt->error]));
            }
            die(json_encode(["Person updated with ID: " . $PersonIdInt]));
        }*/
    ...
    /*if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['FirstName'], $_POST['Surname'], $_POST['DateOfBirth'], $_POST['Email'])) {
            ...
            die(json_encode(['error' => 'Incomplete form data']));
        }
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_PUT['RecordID'], $_PUT['FirstName'], $_PUT['Surname'], $_PUT['DateOfBirth'], $_PUT['Email'])) {
            $EditRecordID = intval($_PUT['RecordID']);
            $EditFirstName = $_PUT['FirstName'];
            $EditSurname = $_PUT['Surname'];
            $EditDateOfBirth = $_PUT['DateOfBirth'];
            $EditEmail = $_PUT['Email'];

            $EditAge = date_diff(date_create($EditDateOfBirth), date_create('now'))->y;
            $PersonObj->savePerson($EditRecordID, $EditFirstName, $EditSurname, $EditDateOfBirth, $EditEmail, $EditAge);
            die(json_encode(['success' => true, 'message' => 'Record updated successfully']));
        } die(json_encode(['error' => 'Invalid form data for updating person']));
    }

    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['deletePerson'], $_POST['PersonId'])) {
...
//            die json_encode(['error' => 'Invalid delete request']);
    }*/
